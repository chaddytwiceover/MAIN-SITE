...
import {
  findNearbyPowerBloom,
  placePowerBlooms,
  type PowerBloomRef,
} from "../systems/powerBlooms";
import {
  clearPowerUps,
...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.powerBlooms.forEach((bloom) => {
      this.physics.add.overlap(this.player.sprite, bloom, () => this.collectPowerBloom(bloom));
    });
  ...
  update() {
    ...
    updateActions();
    const effectiveSpeed = this.level.playerSpeed * getPlayerSpeedMultiplier();
    updatePlayer(this.player, effectiveSpeed);
    updatePowerUps(this, this.player, this.hazards, now);
    updateHazards(this.hazards, px, py, now, isFrostActive());
    updateWater(this.water, now);

    const nearby = findNearbyCollectible(this.flowers, px, py);
    if (nearby) this.collectFlower(nearby);

    const nearbyBloom = findNearbyPowerBloom(this.powerBlooms, px, py);
    if (nearbyBloom) this.collectPowerBloom(nearbyBloom);

    if (!isFrostActive() && hazardTouches(this.hazards, px, py)) {
      this.hitPlayer();
    }
  ...
  private collectPowerBloom(bloom: PowerBloomRef) {
    if (!bloom.active || this.ended) return;
    ...
    triggerPowerUp(this, kind, this.player, this.hazards);
  }

  private hitPlayer() {
    if (this.ended || isFrostActive()) return;
    const now = this.time.now;
  ...
  private finish(result: "won" | "lost") {
    ...
    clearPowerUps(this.player, this.hazards);
    clearBanner();
    setMusicPaused(true);
    if (result === "won") {
