...
export type CollectObjective = {
  type: "collect";
  /** Specific kind, or "any" to count every flower. */
...
export type HazardDef = {
  ...
  speed: number;
  patrol?: Point[];
  /** Bee alert / chase radius in pixels (defaults to 160) */
  detectRadius?: number;
  /** Bee chase speed (defaults to speed * 1.5) */
  chaseSpeed?: number;
  /** Wasp guard home position / radius */
  guardZone?: { x: number; y: number; radius: number };
  /** Wasp leash distance before returning to guard point */
  leashRadius?: number;
};
